...
class LoadStreams:
    ...
    def __init__(self, sources='streams.txt', img_size=640, stride=32, auto=True):
        ...
        while(self.imgs[0] is None):
            self.imgs[0] = cam.get_frame()
        # print(self.imgs[0])
        self.threads[0] = Thread(target=self.update, args=([0,cam]), daemon=True)
    ...
    def update(self, i, cam):
        ...
        n, cam = 0, cam  # frame number, frame array, inference every 'read' frame
        while not cam._frame_que.empty() :
            n += 1
            # _, self.imgs[index] = cap.read()
            im=cam.get_image()
            self.imgs[i] = im
            # time.sleep(1 / 20)  # wait time
    ...
    def __next__(self):
        ...
        img0 = self.imgs.copy()
        # print(img0.shape, "khch2")
        print(self.imgs[0].shape, "khch3")
        print("khch")
        # print(img0)
    ...
    def __len__(self):
         return 1#len(self.sources)   1E12 frames = 32 streams at 30 FPS for 30 years
